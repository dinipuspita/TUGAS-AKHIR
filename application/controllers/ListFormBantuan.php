...
class listFormBantuan extends CI_Controller {
	...
	public function update($id)// sudah di isi di autoloard 
	{
		...
		$this->load->model('List_FormBantuan');
		$data["pengenalan_tempat"] = $this->List_FormBantuan->getPengTempat($id);
		$data["keterangan_pengesahan"] = $this->List_FormBantuan->getPengesahan($id);
		$data["keterangan_perumahan"] = $this->List_FormBantuan->getKetPerum($id);
		$data["keterangan_sosial_ekonomi"] = $this->List_FormBantuan->getSosEko($id);
		$data["kepemilikan_aset"] = $this->List_FormBantuan->getKepAset($id);
		$data['user'] = $this->List_FormBantuan->getUser();
		...
		if($this->form_validation->run() == FALSE) {
			...
			$this->List_FormBantuan->updateById($id);
	...
	public function laporanPenerimaBantuan()
	{
		...
		if($this->form_validation->run() == FALSE) {
			...
			$this->load->model('List_FormBantuan');
			$data["transaksi_bantuan"] = $this->List_FormBantuan->getReportLaporanDinsos();
			if (empty($data['transaksi_bantuan'])) {
