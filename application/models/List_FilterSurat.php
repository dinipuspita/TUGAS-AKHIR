...
class List_FilterSurat extends CI_Model {
	...
	{
	    $query = $this->db->query("Select * from surat AS A Join penduduk AS b ON b.NIK=A.NIK join desa as c on c.id_desa=b.id_desa Join kepemilikan_aset AS d ON d.NIK=A.NIK where status_surat = 'Diterima' and b.usia BETWEEN 45 and 74 AND a.persetujuan='Belum Disetujui'");
	    return $query->result();	   

	}
	public function getDataKIS($kriteria_bantuan)
	{
	    $query = $this->db->query("Select * from surat AS A Join penduduk AS b ON b.NIK=A.NIK join desa as c on c.id_desa=b.id_desa Join kepemilikan_aset AS d ON d.NIK=A.NIK where status_surat = 'Diterima' AND a.persetujuan='Belum Disetujui'"); 
	    return $query->result_array();	   
	}

	public function getDataRASKIN($kriteria_bantuan)
	{
	    $query = $this->db->query("Select * from surat AS A Join penduduk AS b ON b.NIK=A.NIK join desa as c on c.id_desa=b.id_desa Join kepemilikan_aset AS d ON d.NIK=A.NIK where a.status_surat = 'Diterima' and b.usia BETWEEN 75 and 90 AND a.persetujuan='Belum Disetujui'"); 
	    return $query->result();	   
	...
	{
     	$query = $this->db->query("Select * from surat AS A Join penduduk AS b ON b.NIK=A.NIK join desa as c on c.id_desa=b.id_desa Join kepemilikan_aset AS d ON d.NIK=A.NIK where status_surat = 'Diterima' and b.usia BETWEEN 7 and 18 AND a.persetujuan='Belum Disetujui'");
	    return $query->result();   
	...
	// Tambahan untuk ajax
	public function insertBanyak($table, $data){
		$hasil = $this->db->insert_batch($table, $data);
        return $hasil;
	}
	public function updateBanyak($table, $data){
		$hasil = $this->db->update_batch($table, $data, 'id_surat');
        return $hasil;
	}
